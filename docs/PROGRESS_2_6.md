...
Updated: 2026-09-19. This page says how far the 2.6 line has come, item by
...
| --- | --- | --- | --- |
| 1 | The recall process and Cued Recall ship behind a gate | **In research** | Designed in [section 1](RELIABLE_RECALL_2_6.md#1-deliberative-recall-the-recall-process) and [section 2](RELIABLE_RECALL_2_6.md#2-cued-recall-the-input-contract). No implementation on `master`. |
| 2 | The final re-sort has been decided | **In development** | Decided: with confidence enabled, the confidence score no longer re-sorts or gates recall, and `CPERSONA_CONFIDENCE_ORDERING=legacy` restores it. The far weight and an age weight are one prior function that orders only what the gate admitted, both at identity defaults until measured ([design](PRIOR_FUNCTION_DESIGN.md)). On `master`, unreleased. |
...
| 6 | Every benchmark failure has a code and a replayable trace | **In research** | The taxonomy is in [section 8](RELIABLE_RECALL_2_6.md#8-recall-quality-engineering). A replay exists in the benchmark harness; the server does not record a trace yet. |
