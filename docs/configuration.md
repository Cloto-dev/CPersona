...
| `CPERSONA_RRF_LEXICAL_WEIGHT` | `1.0` | Weight on the lexical arms' reciprocal-rank votes under `rrf`. `1.0` is the fusion this server has always shipped, and the vote is computed so that the default is bit-identical to it; `0` leaves dense rows first and lexical-only rows in the tail. It exists because no single constant serves every embedding model — a low weight recovers most of the pure-ranking loss on mid and strong models and costs the weakest model most of its gains — so it is the measured control arm of that comparison rather than a knob to tune by feel. Negative values are clamped to zero |
...
| `CPERSONA_FUSED_GATE_ENABLED` | `true` | The post-fusion quality gate. Disabling it is a last resort: under `rrf`/`rsf` nothing else filters a fused row, because the pool-size heuristic no longer applies to one ([contract §12](behavior-contracts.md#12-what-filters-a-fused-order)); under `cascade` that heuristic still cuts. What you lose either way is the operating point measured for this corpus |
