...
> what you use it for, and links to the contract whenever a tool behaves in a
> way its name does not suggest.
...
| `recall` | Retrieve memories through the three-layer hybrid search. **The last element is the best match** ([ordering contract](behavior-contracts.md#1-recall-return-order-last-is-best)). With `trace: true` it also returns a record of what each stage kept, dropped and reordered, with references and scores but no stored text ([recall trace](RECALL_PROCESS_DESIGN.md#1-the-recall-trace)). With `time_cue` the caller says when the answer was stored, with a confidence; rows found in that period move up by a bounded number of places and one seat is held for a record only that search found, without changing which rows pass the quality gate ([time cue](RECALL_PROCESS_DESIGN.md#2-the-loops-basic-form)) |
...
[carry no score](behavior-contracts.md#7-profile-rows-carry-no-score). That
...
| `delete_agent_data` | Delete **everything** belonging to one agent. It is exposed over the network like any other tool, which is reason enough to set `CPERSONA_AUTH_TOKEN` on the [HTTP transport](configuration.md#remote-http-transport) |
...
| Tool | What it does |
|---|---|
| `export_memories` | Write memories, episodes and profiles to JSONL. It is schema-version independent, so it doubles as a logical backup ([backup runbook](operations.md#backup-and-restore)) |
...
outbound request ([what it sends](architecture.md#transports)).
...
Use these for benchmarking, or for throwaway exploration you do not want in
the corpus.
