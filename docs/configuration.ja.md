<!-- i18n-source: docs/configuration.md@blob:fd89bf8dac5c986b2ccfa22f01f1b4c8c8de858f -->
...
| `CPERSONA_RECALL_DEPTH_FLOOR` | `0` | 想起深度 (Recall Depth): `limit` が何行を求めていても、各検索アームが融合層に渡す候補の最少数。深度は `max(limit, この値)` で、`CPERSONA_RECALL_LIBRARY_MAX_LIMIT` を上限とします。`0` では `limit` と等しく、これは 2.5 系が出荷していた結合そのものです — 設定するまでランキングは何も動きません。`limit` を超えている時は応答に `depth` が載り、5 行の応答がアームごとに 5 件より多い候補から順位付けされたことを呼び出し側が確認できます。融合モードのみ: `cascade` は `limit` 個のスロットを段ごとに埋める方式で、深くする対象のリストを持ちません ([設計](RELIABLE_RECALL_2_6.md#4-depth-is-not-count)) |
