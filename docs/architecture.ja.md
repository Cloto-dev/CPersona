<!-- i18n-source: docs/architecture.md@blob:098378f8f5eec6560b7acc9c1306d638ecc72625 -->
...
- **プロフィール** (`update_profile`) — ユーザーやプロジェクトについて蓄積された
  属性。**スコープ内に 50 件以上の記憶がある場合に**限って recall 応答へ付加され
  (それ未満ではゲートが落とします)、プレビュー切り詰めの対象外で、かつ
...
エージェント間で行を共有しません — かつ `''` の束縛はどの書き込みも生まない値に
マッチするため、軸を決めずに述語を組み立てた内部コードは、開く方向ではなく
**閉じる方向に失敗**します。**軸の省略はその逆で、意図的なエージェント横断走査
