...
> 一緒に配布されます。このページは **29 個のツール**を「何のために手を伸ばすか」で
...
どちらも MCP の外から `python -m cpersona.checkup` としても実行でき、CI ではこの
形を使ってください。実行頻度の指針は
...
**この停止はセッション単位ではなくプロセス単位です。** クライアントが自分の
プロセスを所有する stdio では実質セッション単位ですが、streamable-HTTP の配備では
1 プロセスが全クライアントに応じるため、停止は全員の書き込みを黙らせます — しかも
他のセッションには何も伝わりません。
...
3 つの分離軸は一様には提供されていません。`agent_id` は 29 個のツールのうち
