...
# ---------------------------------------------------------------------------
# What the golden is allowed to disagree with the current code about
# ---------------------------------------------------------------------------
#
# These two lists and the transforms below live HERE, beside the matrix, rather
# than in the replay test, because two readers need them and only one of them
# can import pytest.
#
# `test_equivalence_252.py` applies them before it compares, which is why the
# suite is green. `scripts/capture-behaviour.py` did not, which is why its
# `--check` reported 138 lines of already-reviewed change as if it were news --
# and, worse, why REGENERATING the golden to add one scenario would have written
# all 138 of them in, retiring the recorded evidence these lists exist to keep.
# The two tools were not disagreeing about behaviour; they were answering
# different questions, and only one of them had been told the answer.
