"""Regression tests for fix group I1 (bug-142, bug-143) in cpersona.checkup.

bug-142: the ``--db`` help text advertised a default (``$CPERSONA_DB_PATH`` or
...
bug-143: ``_run`` had no try/finally around ``close_db()``. An exception raised while
...
# --------------------------------------------------------------------------
# bug-142 — help text no longer advertises a default the code never implements
# --------------------------------------------------------------------------


def test_bug142_help_states_true_resolution_order():
...
# --------------------------------------------------------------------------
# bug-142 — unconfigured run prints the resolved absolute DB path to stderr
# --------------------------------------------------------------------------


def test_bug142_unconfigured_run_announces_resolved_db_path(monkeypatch, capsys):
    ...
    def _fake_run(coro):
        # bug-142 is a pre-run notice; do not actually execute the checks here.
...
def test_bug142_explicit_db_does_not_emit_unconfigured_notice(monkeypatch, capsys, tmp_path):
...
# --------------------------------------------------------------------------
# bug-143 — a DB missing the `memories` table exits nonzero instead of hanging
# --------------------------------------------------------------------------


def test_bug143_foreign_db_exits_nonzero_instead_of_hanging(tmp_path):
