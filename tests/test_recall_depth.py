...
async def test_cascade_is_untouched_by_the_floor(monkeypatch):
    ...
    for count in COUNTS:
        out = await M.do_recall(AGENT, QUERY, limit=count)
        assert "depth" not in out, "cascade did not use the configured fusion depth"
    assert [len(p) for p in pools] == list(COUNTS)


@pytest.mark.asyncio
async def test_recent_recall_does_not_report_an_unused_fusion_depth(monkeypatch):
    await _seed()
    monkeypatch.setattr(config, "RECALL_DEPTH_FLOOR", FLOOR)
    out = await M.do_recall(AGENT, "", limit=5)
    assert "depth" not in out, "recent recall did not use the configured fusion depth"
