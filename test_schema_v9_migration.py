...
@pytest.mark.asyncio
async def test_schema_version_constant_is_9():
    """The migration target constant must be 9."""
    assert SCHEMA_VERSION == 9


@pytest.mark.asyncio
async def test_fresh_db_creates_v9_schema():
    """A brand-new DB is stamped v9 with project_id on every data table."""
    async with _TempDB() as tmp:
        ...
        assert await _schema_version(db) == 9
...
async def test_v8_to_v9_migration_preserves_legacy_rows():
    ...
    async with _TempDB() as tmp:
        ...
        # Boot through get_db() — this runs the real v9 migration path.
        db = await database.get_db()
        ...
        ep = await db.execute_fetchall(
            "SELECT summary, project_id FROM episodes WHERE agent_id = 'legacy-agent'"
        )
        assert ep == [("legacy episode", "")]

        # Isolation indexes are created once the columns exist.
        indexes = await _index_names(db)
        assert "idx_memories_isolation" in indexes
        assert "idx_episodes_isolation" in indexes
