...
async def do_check_health(agent_id: str = "", fix: bool = False, checks: list | None = None) -> dict:
    ...
    # 2.5.2b1 (an earlier decision item b1-3): ``status`` is the only verdict. The legacy
...
async def do_migrate_channel_axis(
    ...
    Prepares the knob2 v2 default flip (an earlier decision). Under the historical default
