...
The ``repairable`` contract (2.5.5, an earlier decision)
----------------------------------------------
...
async def check_axis_hygiene(db, agent_id: str, fix: bool) -> list[dict]:
    ...
    # waiver comment (an earlier decision).
    iso = isolation_where(agent_id=None)
    rows = await db.execute_fetchall(
...
async def check_invalid_source_type(db, agent_id: str, fix: bool) -> list[dict]:
    """Detect and (optionally) canonicalise legacy source shapes (an earlier decision, 1b).
    ...
    # 2.5.5 (an earlier decision): this check's local de-escalation became the registry-wide
