...
doing, not the fusion's. All twenty-two tasks, three models, mean NDCG@10 over
subtasks (tasks with more than 200 queries per subtask are
200-query-per-subtask subsamples). Every sampled query matched the live
pipeline row for row: 8,408 identity checks, zero mismatches.

| Model | Pre-gate mean (S2) | Track B mean (S3) | Gate |
|---|---:|---:|---:|
| all-MiniLM-L6-v2 | 50.11 | 48.30 | **−1.80** |
| jina-v5-text-nano | 56.08 | 55.42 | −0.66 |
| bge-m3 | 55.91 | 55.92 | **+0.01** |

The gate's cost is not a property of the regime; it is a property of the model
and the pool. On the strong embedding it is inert across all twenty-two tasks —
its dense rows clear the gate's cosine threshold and fill the top ten on their
own — and it grows as the embedding weakens and the corpus groups shrink. The
tasks it moves by at least 0.2 points on any model:
...
The ten tasks left out move every model by less than 0.2: ConvoMem,
...
jina-v5-text-nano sits in its four 19–20-row corpus groups — and the small
positive entries are the gate removing lexical-tail intruders on larger pools,
which is what it is for. The published Track B figure for EPBench on the mid
model, 3.4 points below the raw embedding, is a fused score 6.8 points above
it that the gate reversed.
