...
1. **Calibrated gate and autocut off; the pool-size gate stays on.** Ranking
   ...
   `CPERSONA_FUSED_GATE_ENABLED=false` (the launcher sets both): both exist
   for contamination prevention in live use and can only remove items from an
   already-ranked list. The launcher does **not** switch off the pool-size
   heuristic gate `do_recall` applies when no calibrated gate is configured,
   so Track B measures the shipped path including it. That gate is inert on
   large pools and destructive on tiny ones — a defect of the shipped path
   rather than of the regime, so the regime is not redefined around it:
   [`measurements/results-pool-size-gate.md`](measurements/results-pool-size-gate.md)
   for what it costs, [the adaptive fusion design](../docs/ADAPTIVE_FUSION_DESIGN.md)
   (D2) for the fix.
...
Track B includes the shipped pool-size gate (doctrine 1). Scoring the same
fused ranking before and after it, on identical candidates, separates what the
gate costs from what the fusion does:

| Model | Pre-gate mean | Track B mean | Gate |
|---|---:|---:|---:|
| all-MiniLM-L6-v2 | 50.11 | 48.30 | −1.80 |
| jina-v5-text-nano | 56.08 | 55.42 | −0.66 |
| bge-m3 | 55.91 | 55.92 | +0.01 |

The cost is a property of the model and the pool, not of the regime: inert on
the strong embedding across all twenty-two tasks, growing as the embedding
weakens and the corpus groups shrink. Per task, and how it arises:
[`measurements/results-pool-size-gate.md`](measurements/results-pool-size-gate.md).
