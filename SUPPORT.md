...
| 2.6.x | **Experimental** | — | — | Opt-in alpha releases. The first increment improves FTS query punctuation handling; the second adds the experimental `reconstruct` tool and separates recall depth from the response count; the third quotes long records from the part that matches the query, expands part of a record, and was measured with an answer reader. None is the complete 2.6 roadmap. The 2.5.x maintenance branch and production soak remain separate. |
