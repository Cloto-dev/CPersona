...
| 2.5.x | **Current** | not yet | — | Latest release: 2.5.12. Passed the full release gate (test suite, lint, issue-registry verification, audits); all fixes land here. Not yet frozen; certification has not started. |
| 2.6.x | **Experimental** | — | — | Opt-in alpha releases. The first increment improves FTS query punctuation handling; the second adds the experimental `reconstruct` tool and separates recall depth from the response count; the third quotes long records from the part that matches the query, expands part of a record, and was measured with an answer reader; the fourth adds declared associative memory — entities, aliases and relations an agent declares, read by `reconstruct` and the new `traverse` tool; the fifth adds block reach — clause-sized blocks with one sign-quantised vector each, which let recall reach the text of a long record past the embedding window. It is off by default and adds rows after the ranked results rather than reordering them; the sixth re-ranks block candidates by a stored byte-per-dimension vector, shows the part of a record that matched beside a cut `recall` preview and as a `reconstruct` item's head, and gives `reconstruct` the block reservation beside its window; the seventh adds an on-request recall trace and a time cue that moves rows found in a stated period up by a bounded number of places without changing which rows pass the quality gate, stops the confidence score from re-sorting or gating recall, and turns the episode-boundary penalty off by default. None is the complete 2.6 roadmap. The 2.5.x maintenance branch and production soak remain separate. |
