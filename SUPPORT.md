...
| 2.6.x | **Experimental** | — | — | Opt-in alpha releases. The first increment improves FTS query punctuation handling; the second adds the experimental `reconstruct` tool and separates recall depth from the response count. Neither is the complete 2.6 roadmap. The 2.5.x maintenance branch and production soak remain separate. |
